...
The `docusaurus-plugin-openapi-docs` package extends the Docusaurus CLI with commands for generating MDX using the OpenAPI specification as the source. The resulting MDX is fully compatible with [plugin-content-docs](https://docusaurus.io/docs/api/plugins/@docusaurus/plugin-content-docs) and can be used to render beautiful reference API docs when combined with the `docusaurus-theme-shadcn-docs` theme.
...
```bash
yarn add docusaurus-theme-shadcn-docs
```

## Configuring `docusaurus.config.ts` (Plugin and theme usage)

Here is an example of properly configuring `docusaurus.config.ts` for `docusaurus-plugin-openapi-docs` and `docusaurus-theme-shadcn-docs` usage.
...

{
  ...
  themes: ["docusaurus-theme-shadcn-docs"], // export theme components
}
```

> Note: You may optionally configure a dedicated `@docusaurus/plugin-content-docs` instance for use with `docusaurus-theme-shadcn-docs` by setting `docItemComponent` to `@theme/ApiItem`.
...
The `docusaurus-theme-shadcn-docs` theme can be configured with the following options in `themeConfig.api`:
